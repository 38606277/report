...
@RestController
@RequestMapping("/reportServer/dynamicReport")
public class DynamicReportControl {
	...
	private void showAllFiles(File dir, JSONArray aNode){
		...
		for (int i = 0; i < fs.length; i++) {
			...
			if (fs[i].isHidden()||fs[i].getName().endsWith(".xlsx")||
					(fs[i].isDirectory()&&fs[i].getName().equals("static"))) {
				continue;
			}
			...
			while (iter.hasNext()) {
				...
				if (file != null) {
					...
					if (myFileName.trim() != "") {
						...
						convertToDynamicHtml(localFile);
					}
				}
				// 记录上传该文件后的时间
